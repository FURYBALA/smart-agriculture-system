...
| H11 | Irrigation node boots in Wokwi simulation | Serial shows a deterministic boot marker (`WOKWI_IRRIGATION_READY`) confirming the real firmware booted and Serial is alive | Actually run with a real token, root cause found and fixed: `diagram.json` was missing a serial-monitor connection (isolated by testing against Wokwi's own official example project), on top of 2 earlier real config bugs (invalid pin names, missing required `firmware` field). With all three fixed, the real compiled firmware boots and prints the marker, matched by `wokwi-cli --expect-text`, exit code 0 — reproduced locally (Windows) **and** independently on a clean `ubuntu-latest` GitHub Actions runner via `.github/workflows/wokwi-simulation.yml` (real run: [`32560846142`](https://github.com/FURYBALA/smart-agriculture-system/actions/runs/32560846142), `success`). (Wi-Fi itself never connects in this simulation, by design — `config.h`'s SSID is an intentional real-deployment placeholder, not a Wokwi network, so "REST API server started" is never reached; boot + Serial is what's actually verifiable here) | LOCAL WOKWI: PASS. GITHUB ACTIONS WOKWI: PASS — see `wokwi-simulation.md` |
