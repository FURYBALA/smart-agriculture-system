...
- **No physical ESP32, mobile device, or AWS account was available** to
  build this against — so alongside the real implementation, this repo
  also has a local ESP32 REST simulator and a moto-based AWS test
  harness that exercise the real app and backend code end-to-end
  without either. Both are dev/test aids, not a substitute for the
  physical/cloud validation still pending — see
